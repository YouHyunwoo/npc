...
export class Label extends View {
    ...
    constructor({
        ...
        super({
            eventHandling: Label.TargetPolicy.Ignore,
            rendering: Label.TargetPolicy.Self,
            updating: Label.TargetPolicy.Ignore,
            ...args
        });
    ...
    evaluateWrapSizeSelf() {
        ...
        if (this._size[0] === Size.Wrap) {
            this._realSize[0] = this.#textMetrics.actualBoundingBoxLeft + this.#textMetrics.actualBoundingBoxRight + this.padding * 2;
        }

        if (this._size[1] === Size.Wrap) {
            this._realSize[1] = this.#textMetrics.actualBoundingBoxAscent + this.#textMetrics.actualBoundingBoxDescent + this.padding * 2;
        }
    }
    ...
    #getOffset() {
        ...
        if (this.textBaseline === 'top') {
            offsets[1] = this.#textMetrics.actualBoundingBoxAscent; // 위로 삐져나온 정도만큼 아래로 내려가야 함
        }
        else if (this.textBaseline === 'middle') {
            offsets[1] = (this.#textMetrics.actualBoundingBoxAscent - this.#textMetrics.actualBoundingBoxDescent) / 2;
        }
        else if (this.textBaseline === 'bottom') {
            offsets[1] = -this.#textMetrics.actualBoundingBoxDescent; // 아래로 삐져나온 정도만큼 위로 올라가야 함
        }

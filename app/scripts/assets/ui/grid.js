...
export class Grid extends View {
    ...
    evaluateWrapSizeSelf() {
        if (this._size[0] === View.Size.Wrap) {
            if (this._objects.legnth <= 0) {
                ...
                this._realSize[0] = (this.itemSize[0] + this.itemPadding) * Math.min(this._objects.length, this.cols) - this.itemPadding;
            }
    ...
    #align() {
        ...
        for (let r = 0, i = 0; r < this.rows && i < this._objects.length; r++) {
            for (let c = 0; c < this.cols && i < this._objects.length; c++, i++) {
